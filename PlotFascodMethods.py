...
if __name__ == "__main__":
    ...
    temperaturverschiebung = 30

    #==============================
    # 1. Plot: Temperatur
    #==============================
    ax1.semilogy(atm["t"],atm["p"],label="$T_0$")
    ax1.semilogy(np.add(atm["t"],temperaturverschiebung),atm["p"],label="$T_1 = T_0 + %i\,K$"%temperaturverschiebung)

    ax1.invert_yaxis()
    ax1.set_ylim(100000,0)

    ax1.set_xlabel("Temperatur [K]", fontsize=font)
    ...
    ax1.set_title("Temperaturverscheibung",fontsize=font)
    ...
    ax2.loglog(atm["H2O"],atm["p"],label="$H2O_0 bei T_0$",color="red")
    ...
    ax2.loglog(atm["H2O"],atm["p"],label="$H2O_0 bei T_1$",ls="dashed",color="red")

    RH = VMR2RH(atm["H2O"],atm["p"],atm["t_new"])
    RH = np.multiply(RH,3)
    VMR = RH2VMR(RH,atm["p"],atm["t_new"])
    ax2.loglog(VMR,atm["p"],label="$H2O_1 bei T_1$",ls="dashed",color="green")


    ax2.set_xlabel("Wasserdampfgehalt [VMR]",fontsize=font)
    ...
    ax3.loglog(atm["H2O"],atm["p"],label="$H2O_0$",color="red")

    RH = VMR2RH(atm["H2O"],atm["p"],atm["t"])
    RH[:3] = 0.90
    VMR = RH2VMR(RH,atm["p"],atm["t"])
    ax3.loglog(VMR,atm["p"],label="$H2O_1$",color="green")


    atm["t_new"] = np.add(atm["t"],temperaturverschiebung)


    ax3.loglog(atm["H2O"],atm["p"],label="$H2O_0 bei T_1$",ls="dashed",color="red")
    ...
    ax3.set_xlabel("Wasserdampfgehalt [VMR]",fontsize=font)
